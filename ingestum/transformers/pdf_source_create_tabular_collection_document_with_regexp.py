...
class Transformer(BaseTransformer):
    ...
    returns a `Collection` of `Tabular` documents for each.

    :param start_regexp: Regexp to identify the start of tables in the PDF text
    :type start_regexp: str
    :param end_regexp: Regexp to identify the end of tables in the PDF text
    :type end_regexp: str
    ...
    def find_coords(self, layout, pageno):
        ...
        end_regexp = re.compile(self.arguments.end_regexp)
        curr_table = None
        ...
        for lobj in layout:
            ...
            if curr_table is not None:
                if isinstance(lobj, LTTextBox) and end_regexp.search(lobj.get_text()):
                    tables.append(curr_table)
                    curr_table = None
                    continue
