...
class OpenCVReader(BaseReader):
    ...
    def find_boxes(self, img):
        ...
        OpenCV. The tuples are in the form (x, y, width, height)."""
        # Invert the image
...
class Transformer(BaseTransformer):
    ...
    environment variables for authentication if using the Adobe PDF Extract API.
    ...
    def extract(self, source, extractables=None, replacements=None):
        ...
        for pageno, page in enumerate(pages, start=first_page):
            ...
            # Import all BaseReader classes, including plugins
            reader_classes = utils.find_subclasses(BaseReader)
            Reader = next(
                (
                    r
                    for r in reader_classes
                    if r.__fields__["type"].default == self.arguments.reader
                ),
                None,
            )
            if Reader is None:
                raise ValueError(f"{self.arguments.reader} does not exist")
            r = Reader()
            boxes = r.find_boxes(img)
